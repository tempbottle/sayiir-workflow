---
title: Roadmap
description: What's planned for Sayiir — conditional branching, workflow context, loops, observability, child workflows, edge runtimes, and more
---

import { Card, CardGrid } from '@astrojs/starlight/components';

Sayiir is a durable workflow engine. It's great at what it does: checkpointed execution, parallel tasks, retries, signals, and crash recovery — all with zero infrastructure.
...
Today, Sayiir workflows are static DAGs: the graph is fixed at build time. You can't route to different tasks based on a previous task's output. This is the single biggest gap for AI agent and business process use cases.

**What it enables:**
- Route to different sub-agents based on LLM classification
- Handle success/failure paths differently
- Skip expensive steps when results are already cached
...
workflow = (
    ...
    .branch_on(
        lambda result: result["intent"],
        {
            "billing": billing_flow,
            "technical": tech_support_flow,
...
Sayiir's DAG model is acyclic by definition — no loops. This prevents iterative patterns like "try a tool, check if the result is good enough, try again if not."

**What it enables:**
- Iterative AI agents that refine their output
- Polling workflows that wait for external conditions
- Retry-with-different-strategy patterns

**Proposed API:**

```python
workflow = (
    Flow("iterative-agent")
    .then(initial_attempt)
    .loop(
        refine_and_check,
        until=lambda result: result["quality_score"] > 0.8,
        max_iterations=5,
    )
    .then(finalize)
    .build()
...
- [Join the Discord](https://discord.gg/MWSzsHeg) to discuss design decisions
- PRs welcome — especially for observability and conditional branching
