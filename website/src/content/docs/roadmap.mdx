...
### AWS Backends (DynamoDB + SQS)

Native AWS persistence and task claiming for teams running on AWS infrastructure.

- **DynamoDB backend** — Serverless persistence with on-demand scaling, no database to manage
- **SQS-based task claiming** — Replace PostgreSQL polling with SQS message-driven task distribution
- Fully serverless workflow execution when combined with Lambda
- Same `Backend` trait — swap in with zero workflow code changes
