...
```typescript
import { task, flow } from "sayiir";

const createOrder = task("create_order", (orderId: number) => {
    return { order_id: orderId, status: "pending" };
});

const fulfill = task("fulfill", (approval: { by?: string }) => {
    return `Fulfilled order (approved by ${approval.by ?? "unknown"})`;
});

const workflow = flow<number>("approval")
    .then(createOrder)
    .waitForSignal<{ by: string }>("approval_wait", "manager_approval", {
        timeout: "24h",
    })
    .then(fulfill)
    .build();
...
```typescript
import { sendSignal, InMemoryBackend } from "sayiir";
...
```typescript
import { runDurableWorkflow, sendSignal, resumeWorkflow } from "sayiir";

// Start the workflow
const status1 = runDurableWorkflow(workflow, "order-42", 42, backend);
console.log(status1.status); // "awaiting_signal"

// Signal can be sent immediately or hours later
sendSignal("order-42", "manager_approval", { by: "alice" }, backend);

// Resume to continue execution
const status2 = resumeWorkflow(workflow, "order-42", backend);
console.log(status2.status); // "completed"
```
...
const workflow = flow<number>("approval")
    .then(createOrder)
    .waitForSignal("approval_wait", "manager_approval", { timeout: "24h" })
    .then(fulfill)
    .build();
...
const fulfill = task("fulfill", (approval: { by?: string }) => {
    const approver = approval.by ?? "unknown";
    console.log(`Fulfilling order (approved by ${approver})`);
    return `Fulfilled order (approved by ${approver})`;
});
...
// Start workflow
const status1 = runDurableWorkflow(workflow, "order-42", 42, backend);
console.log(`Status: ${status1.status}`); // "awaiting_signal"
