...
// Option A: explicit worker ID
let worker = PooledWorker::new("worker-1", backend, TaskRegistry::new())
    .with_claim_ttl(Some(Duration::from_secs(300)));

// Option B: auto-generated ID from hostname + PID (e.g. "myhost-12345")
// let worker = PooledWorker::builder(backend, TaskRegistry::new())
//     .claim_ttl(Some(Duration::from_secs(300)))
//     .build();
...
runner.run(&workflow, "order-42", input).await?;
```
