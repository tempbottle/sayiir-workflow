...
- `max_delay` *(optional)*: Cap on the backoff duration to prevent unbounded growth
...
CallApi::register(&mut registry, codec.clone(), CallApi::new());
Process::register(&mut registry, codec.clone(), Process::new());

let workflow = WorkflowBuilder::new(ctx)
    .with_existing_registry(registry)
    .then_registered::<serde_json::Value>(CallApi::task_id())?
    .then_registered::<String>(Process::task_id())?
    .build()?;
```
