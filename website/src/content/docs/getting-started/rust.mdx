...
2. Calls `verify_deps` for each task **before** construction. If any task asks for a type that wasn't inserted, the macro returns a `BuildErrors::MissingDep { task_id, type_name }` rather than panicking at first invocation.
