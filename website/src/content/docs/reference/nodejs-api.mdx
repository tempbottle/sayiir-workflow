...
const configured = task("configured-task", async (input: string) => {
...
- `fn` ((input: TIn) => TOut | Promise\<TOut\>) — The task function
...
- `fn` ((input: TLast) => TOut | Promise\<TOut\>) — Task function
...
- `opts.schema` (ZodLike\<TSignal\>, optional) — Zod schema for signal validation
...
Tasks can be `async` functions. Sayiir detects promises automatically:
