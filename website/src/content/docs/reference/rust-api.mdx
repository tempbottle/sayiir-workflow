...
Type-keyed dependency-injection container shared by `#[task]` and `workflow!`. Re-exported by `sayiir-core` (as `sayiir_core::deps`) and `sayiir-runtime`, so most users do not need to add this crate directly.

**Key Types:**
- `Deps` — type-keyed service container
- `DepsBuilder` — fluent builder
- `MissingDep` — error type returned by `try_get` and listed by `Task::verify_deps`
...
- `register_from_deps(&mut reg, codec, &deps) -> Result<(), Vec<MissingDep>>` — verify, build, and register in one call. Used by task libraries and `PooledWorker` setup where the `workflow!` macro is not the entry point.
...
| `deps` | no | `&Deps` container — when set, tasks are built via `Task::from_deps(deps)` and missing dependencies surface as `BuildErrors::MissingDep` at construction time. |
