...
### run_workflow(workflow, input_data, *, instance_id=None, backend=None)
...
**Raises:** `WorkflowError` if durable workflow did not complete. Use `run_durable_workflow()` when you need the full `WorkflowStatus` object.
