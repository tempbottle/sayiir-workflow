//! Internal helpers for snapshot ↔ database field extraction.
...
/// Whether the snapshot is in a terminal state (needs `completed_at` set).
...
/// since those don't have a meaningful position.
