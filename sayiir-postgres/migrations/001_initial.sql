...
CREATE INDEX IF NOT EXISTS idx_snapshots_task    ON workflow_snapshots (current_task_id);
CREATE INDEX IF NOT EXISTS idx_snapshots_updated ON workflow_snapshots (updated_at);

-- Snapshot history: append-only log of every checkpoint (1:N per instance).
CREATE TABLE IF NOT EXISTS workflow_snapshot_history (
...
CREATE INDEX IF NOT EXISTS idx_history_instance ON workflow_snapshot_history (instance_id, version);
